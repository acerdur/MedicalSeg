...
from torch import nn, float32 as torchfloat32, optim, save, device
...
experiment_name = f"{model_architecture}_{model_name}_{task}_{timestamp}"
# %%
...
train, val = (
    ...
    DataLoader(
        val, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True
    ),
)
...
trainer = pl.Trainer(
    ...
    callbacks=[
        ...
        pl.callbacks.early_stopping.EarlyStopping(monitor="val_loss", patience=40),
...
# %%
save(model.model.state_dict(), f"models/best_{experiment_name}.pth")
